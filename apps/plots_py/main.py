...
async def render(values: list[float] = Body(..., embed=False)):
    ...
    if not isinstance(values, list) or len(values) == 0:
        raise HTTPException(
            status_code=400, detail="expected non-empty array of numbers"
        )
...
async def render_csv(request: Request):
    ...
    # keep the 400 on decode failure
    try:
    ...
    with io.StringIO(text, newline="") as f:
        ...
        for row in rdr:
            for field in row:
                ...
                if not s:
                    continue
                # E722 fix: catch specific errors; also ignore non-finite
    ...
    # reuse render behavior: generate PNG
